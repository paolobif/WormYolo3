...
if __name__ == "__main__":
    ...
    parser.add_argument("--sort", action='store_true', help="try running deepsort")
    parser.add_argument("--minigpu", default = False, action='store_true', help="reduces number of input frames at once to GPU")
    ...
    if INPUT_VIDEO == True:
        ...
        while (1):
            ...
            if opt.minigpu == False: #BB: if/else statement depending on fullsize or mini GPU
                ...
                for key in input_dict: #BB: for loop iterating through input dict
                    input_list = list(input_dict.keys()) #BB: create list of keys
                if (len(input_list) % 2 == 0): #BB: check if length of keys is divisible by 2
                    n = 2 #BB: set size of image batch
                elif (len(input_list) % 3 == 0): #BB: check if length of keys is divisible by 3
                    n = 3 #BB: set size of image batch
                else:
                    n=5 #BB: set size of image batch   
                final = [input_list[i * n:(i + 1) * n] for i in range((len(input_list) + n - 1) // n )]  #split list of keys into chunks
                outputs = [] #BB: create empty value for outputs to be added to
                for item in range(len(final)): #BB: iterate through each sub dict
                    mini_list = final[item] #BB: create smaller list of keys
